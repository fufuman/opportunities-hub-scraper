...
BASE_URL = "https://art.uga.edu/directory/"
NUM_PAGES = 10  # confirmed via pagination links (1 2 3 ... 10) on page 1, 2026-08-09

CACHE_DIR = os.path.join("..", "cache", "uga_faculty")

CARD_SPLIT_RE = re.compile(r'(?=<h2 class="font-medium)')
NAME_RE = re.compile(r'<h2 class="font-medium[^>]*>([^<]+)</h2>', re.DOTALL)
TITLE_RE = re.compile(r'<p class="ml-0[^"]*"[^>]*>([^<]*)</p>', re.DOTALL)
EMAIL_RE = re.compile(r'<strong>Email</strong>\s*([^<]*)</p>', re.DOTALL)
AREA_RE = re.compile(r'<strong>Academic Area</strong>\s*([^<]*)</p>', re.DOTALL)

MEDIUM_KEYWORDS = {
    "Painting/Drawing": ["painting", "drawing"],
    "Sculpture": ["sculpt", "ceramic", "jewelry", "metalwork", "metal"],
...
def parse_faculty(page_html, source_url):
    ...
    cards = []
    for c in raw_cards:
        next_idx = c.find('<h2 class="font-medium', 1)
    ...
    for card in cards:
        ...
        medium = classify_medium(f"{area} {title}")
        if not medium:
            continue
...
def main():
    parser = argparse.ArgumentParser(description="Scrape UGA Lamar Dodd School of Art faculty emails")
    parser.add_argument("--out", default="uga_faculty.csv")
